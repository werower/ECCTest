...
public class SelfSignedCertificateGenerator {
    ...
    private static SubjectPublicKeyInfo getPublicKeyInfo(PublicKey publicKey) {
        if (!(publicKey instanceof ECPublicKey))
            throw new RuntimeException("publicKey is not an ECPublicKey");
        
        try {
      
        	AsymmetricKeyParameter pubkeyParam = PublicKeyFactory.createKey(publicKey.getEncoded());
        	return SubjectPublicKeyInfoFactory.createSubjectPublicKeyInfo(pubkeyParam	);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private static ContentSigner getSigner(KeyPair keys) {
        try {        
			ContentSigner sigGen = new JcaContentSignerBuilder(SIGNATURE_ALGORITHM).build(keys.getPrivate());
            return sigGen;
        	//return new JcaContentSignerBuilder(SIGNATURE_ALGORITHM).setProvider(new BouncyCastleProvider()).build(keys.getPrivate());
